...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/items/autocomplete`, { params });
  return response.data;
}

const ItemsSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.items.hasPermissionToCreate,
});

export default connect(select)(ItemsSelectItem);
