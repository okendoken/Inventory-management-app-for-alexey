...
import * as dataFormat from 'pages/CRUD/Items/table/ItemsDataFormatters';

import actions from 'actions/items/itemsListActions';
...
const ItemsTable = () => {
  ...
  const count = useSelector((store) => store.items.list.count);
  const modalOpen = useSelector((store) => store.items.list.modalOpen);
  const rows = useSelector((store) => store.items.list.rows);
  const idToDelete = useSelector((store) => store.items.list.idToDelete);
  ...
  return (
    <div>
      <Widget title={<h4>{humanize('Items')}</h4>} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/items/new' className={classes.element}>
        ...
        >
          <DataGrid
            ...
            loading={loading}
            onRowClick={(e) => {
              history.push(`/admin/items/${e.id}/edit`);
            }}
            autoHeight
          />
        </div>
          ...
          <LinkMaterial
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Items'
                : 'http://localhost:8080/api-docs/#/Items'
            }
          >
            API documentation for items
          </LinkMaterial>
        </div>
      </Widget>
